# R script to figures in "Hvordan klarer Alternativet sig i meningsmålingerne? #2"
# Link: https://erikgahner.dk/2020/hvordan-klarer-alternativet-sig-i-meningsmalingerne-2/
...
polls %>%
  drop_na(party_aa) %>% 
  ggplot(aes(x=as.Date(date), y=party_aa, colour = pollingfirm)) + 
  geom_hline(yintercept = 2) +
  ggthemes::scale_color_colorblind() +
  geom_point(size=3, alpha = 0.5) +
  labs(y = "Opbakning til Alternativet (%)",
       x = NULL) +
  theme(legend.position="bottom")

ggsave("alternativet.png", width = 7, height = 5)
