# R script to figures in "Hvor mange vil stemme på Veganerpartiet?"
# Link: https://erikgahner.dk/2020/hvor-mange-vil-stemme-pa-veganerpartiet/
...
polls %>%
  drop_na(party_g) %>% 
  ggplot(aes(x=date, y = party_g, ymin = party_g - ci_g, ymax = party_g + ci_g, colour = pollingfirm)) + 
  geom_hline(yintercept = 2, linetype = "dashed") +
  geom_line() +
  geom_point(size=3) +
  geom_errorbar(width = 0) +
  labs(y = "Opbakning til Veganerpartiet (%)",
       x = NULL) +
  theme(legend.position="bottom") +
  ggthemes::scale_color_colorblind() 

ggsave("veganerpartiet.png", width = 6, height = 4)

number_of_polls <- polls %>% drop_na(party_g) %>% nrow()
...
newpolls <- bind_rows(polls_aa, polls_d, polls_e, polls_p)
...
ggsave("nyepartier.png", width = 9, height = 7)
